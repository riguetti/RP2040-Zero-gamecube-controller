	...
	if (vertical && horizontal) {
		if (bs.l || bs.r) {
			if (bs.mx == bs.my)
				...
				xy = (banParasolDashing && readUp) ?
						coords(0.475, 0.875) : coords(0.5, 0.85);
		...
		if (bs.mx == bs.my)
			...
			xy = (buttonSet.left && buttonSet.right) ?
					coords(1.0, 0.0) : coords(0.6625, 0.0);
		else
			xy =
					((banSlightSideB && bs.b)
							|| buttonSet.left && buttonSet.right) ?
							coords(1.0, 0.0) : coords(0.3375, 0.0);
