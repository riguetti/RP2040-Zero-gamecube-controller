...
void convertToPio(const uint8_t *command, const int len, uint32_t *result,
		int &resultLen) {
	if (len == 0) {
		resultLen = 0;
		return;
...
void enterMode(int dataPin, std::function<GCReport()> func) {
	...
	while (true) {
		...
		if (buffer[0] == 0) { // Probe
			...
			gpio_put(25, 1);
			uint8_t originResponse[10] = { 0x00, 0x80, 128, 128, 128, 128, 0, 0,
					0, 0 };
			uint32_t result[6];
			int resultLen;
